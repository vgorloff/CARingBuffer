...
class CASwiftRingBufferTests: XCTestCase {
	...
	let x = CARingBuffer<Float>(nChannels: 2, capacityFrames: 32)
	...
	override func setUp() {
		...
		ringBuffer = CARingBuffer<Float>(nChannels: 2, capacityFrames: 5)
	}
	...
	func testIOInRange() {
		...
		var status = CARingBufferErrorCode.OK
		...
		XCTAssertTrue(compareBuffersContents(writeBuffer, writeBufferOffset: 0, readBuffer: readBuffer, readBufferOffset: 0, numberOfFrames: 2))
		status = ringBuffer.GetTimeBounds(startTime: &startTime, endTime: &endTime)
		XCTAssertTrue(status == .OK)
		XCTAssertTrue(startTime == 0 && endTime == 4)
		...
		XCTAssertTrue(compareBuffersContents(writeBuffer, writeBufferOffset: 2, readBuffer: readBuffer, readBufferOffset: 0, numberOfFrames: 2))
		status = ringBuffer.GetTimeBounds(startTime: &startTime, endTime: &endTime)
		XCTAssertTrue(status == .OK)
		XCTAssertTrue(startTime == 0 && endTime == 4)
	}
	...
	func testReadBehindAndAhead() {
		...
		XCTAssertTrue(status == .OK)
		printChannelData(title: "Read buffer (1 part):", buffer: readBuffer)
		XCTAssertTrue(compareBuffersContents(writeBuffer, writeBufferOffset: 0, readBuffer: readBuffer, readBufferOffset: 2, numberOfFrames: 2))
	...
	func testWriteBehindAndAhead() {
		...
		XCTAssertTrue(compareBuffersContents(writeBuffer, writeBufferOffset: 0, readBuffer: readBuffer, readBufferOffset: 2, numberOfFrames: 4))
		XCTAssertTrue(checkBuffersContentsIsZero(readBuffer, bufferOffset: 0, numberOfFrames: 2))
	...
	func testIOWithWrapping() {
		...
		printChannelData(title: "Read buffer:", buffer: readBuffer)
		XCTAssertTrue(checkBuffersContentsIsZero(readBuffer, bufferOffset: 0, numberOfFrames: 2))
		XCTAssertTrue(compareBuffersContents(secondaryWriteBuffer, writeBufferOffset: 2, readBuffer: readBuffer, readBufferOffset: 2, numberOfFrames: 2))
		XCTAssertTrue(compareBuffersContents(writeBuffer, writeBufferOffset: 0, readBuffer: readBuffer, readBufferOffset: 4, numberOfFrames: 6))
	}
