...
public struct Assert: AssertType {
   ...
   public static func verify(_ screenshot: Screenshot, file: StaticString = #file, line: UInt = #line) {
      if TestSettings.shared.testEnvironment.isReferenceTest {
         do {
            try screenshot.saveReference()
         } catch {
            XCTFail(String(describing: error), file: file, line: line)
         }
         return
      }
      let fixture = screenshot.localFixtureFilePath
      if !screenshot.isFixtureExists {
         XCTFail("Fixture for screenshot is not yet exists at \"\(fixture)\"", file: file, line: line)
         return
      }
      do {
         let tolerance: Screenshot.Percentage = 0.0 // Value in `%`
         let status = try screenshot.compare(tolerance: tolerance)
         if !status {
            let message: String
            if tolerance == 0 {
               message = "Screenshot not equal to fixture: \(fixture)."
            } else {
               message = String(format: "Screenshot exceeded tolerance %.3f%% when comparing with fixture: \(fixture).",
                                tolerance)
            }
            XCTFail(message, file: file, line: line)
         }
